...
from command.mailtools import send_mail, set_mail, verify_mail, set_mail_limit_mb, set_mail_limit_kb, set_mail_delay
...
async def process_command(client: Client, message: Message, active_cmd: str, admin_cmd: str, user_id: int, username: str, chat_id: int):
    ...
    if text.startswith("/start"):
    ...
    elif text.startswith(("/setmail", "/sendmail", "/verify", "/setmb","/setkb", "/setdelay")):
        if cmd("mailtools", user_id in admin_users, user_id in vip_users):
            if text.startswith("/setmail"):
                ...
                await asyncio.create_task(set_mail_limit_mb(client, message))

            elif text.startswith("/setkb"):
                await asyncio.create_task(set_mail_limit_kb(client, message))
