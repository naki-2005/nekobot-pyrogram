...
async def process_command(client, message, user_id, username, chat_id, int_lvl):
    ...
    if command == "/start":
        ...
        try:
            result = subprocess.run([desmega_path, mega_url, "--path", output_dir], stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
            ...
            for file_name in files:
                ...
                if file_size_mb > 2000:
                    ...
                    cmd_args = [seven_zip_exe, 'a', '-mx=0', f'-v2000m', archive_base, file_path]
                    zip_result = subprocess.run(cmd_args, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
                    if zip_result.returncode != 0:
                        await message.reply(f"❌ Error al comprimir {file_name}:\n{zip_result.stderr}")
                        continue
                    os.remove(file_path)
                    archive_parts = sorted([f for f in os.listdir(output_dir) if f.startswith(f"{file_name}_archive.7z")])
